...
The technical activities of the Internet community are summarized in documents knows RFCs. In addition to standardizing the Internet protocols, RFCs sometimes just document or explain aspects of existing practice. RFCs
...
| Transport | TCP      | Provide reliable, full-duplex, error-corrected communication                                      | [9263](https://www.rfc-editor.org/rfc/rfc9293)                                                                                                                                                 |
...
| Layer   | Protocol | Description                                                                                    | RFC                                                                                                       |
| ------- | -------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Network | IP       | Route packets from one host to another                                                         | [791](https://www.rfc-editor.org/rfc/rfc791.html),<br>[8200](https://www.rfc-editor.org/rfc/rfc8200.html) |
...
As a packet travels down the protocol stack, each protocol add its own header. The packet generated by a protocol becomes the payload of the next one. On the receiver, the encapsulation is reversed as the packet travels backup the protocol stack

---

Consider an application that transmits data over UDP. Application data becomes the payload of the UPD packet. UDP adds its own header information, i.e., source port, destination port, length, and checksum
...
| Addressing | Subject                                                                                                                                                                                        |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MAC        | For use by hardware. An NIC typically has one MAC address, which is assigned at the time of manufacture                                                                                        |
| IP         | For use by software. IP addresses are hardware independent. Within any particular network context, an IP address identifies an NIC (not a computer)                                            |
...
| Port       | Valid ports are in the range 1 - 65535. A port identifies a processes running on a computer. UNIX systems restrict programs from binding to port numbers under 1024, unless they run as `root` |
...
An explicit netmask of `/24` turns a single class B network address into 256 distinct networks. Each network can support 256 host addresses, which can accomodate 254 actual hosts
...
1. The number of host per network and the value of the last byte in the netmask always add up to 256
...
The site's boarder router runs a system called NAT to allow the hosts that use private IPv4 addresses to talk to the Internet
...
Consider a site that has a single public IPv4 address, say `203.0.113.5` and suppose an outbound packet arrives at the site's boarder router from `192.168.1.10:12345`
1. The site's boarder router must change (at least) the source address, otherwise the recipient won't be able to reply (`192.168.1.10` is a private address). This is where NAT comes into play. For example, `192.168.1.10:12345` $\rightarrow$ `203.0.113.5:55231`
2. The recipient will reply back to `203.0.113.5:55231`, which is the public IPv4 address of the site's boarder router that, in turn, will perform the reverse translation, i.e., `203.0.113.5:55231` $\rightarrow$  `192.168.1.10:12345` 

### 2.6. Routing

Routing is the process of directing a packet through the networks that stand between its source and its destination

Routing is similar to asking for directions in an unfamiliar city (with no Internet connections). The first person you talk to might point you toward the right city. The next persone might be able to tell you how to get to the right street. Eventually, you get close enough that someone can identify the building you are looking for
...
1. When someone (say, `192.168.1.50`) does not know the MAC address of the destination IP (say, `192.168.1.1`) broadcasts an ARP request to the local network. For example, `Who has 192.168.1.1?  Tell 192.168.1.50` 
2. `192.168.1.1` replies with its own MAC address in an ARP reply message
3. Upon receiving the ARP reply , `192.168.1.50` updates its cache of known neighbors (`ip neigh` shows these entries)
...
Run (or reboot the system) to apply `99-icmp-redirects.conf` immediately 
...
By default, uRPF is loosely (`2`) enabled
...
The `ping` command sends an ICMP echo requests to a target host and waits to see if the host answers back
...
If your network is in a bad shape, chances are that DNS is not working. Use the numeric IP addresses when pinging, with the `-n` option to prevent reverse lookup on IP addresses, which also trigger DNS requests

---

Remember that
- Some networks block ICMP echo requests with a firewall
- It is hard to distinguish the failure of a network from the failure of a server. A failed ICMP echo requests just tell you that something is wrong
...
A packet sniffer is a tool that listens to network traffic and record or print packets that meet criteria of your choice. It is a good idea to take an occasional sniff of your network to make sure the traffic is in order
...
As packet sniffers read data from a raw network device, so they must run as `root`
...
Both Wireshark and `tcpdump` uses the same underlying `libpcap` library, but Wireshark has more features, such as
...
| Term                                     | Meaning                                                                                                                                                                                            |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Address Resolution Protocol (ARP)        | A link protocol that translate IP addresses to MAC addresses                                                                                                                                       |
...
| Best current practice (BCP)              | A type of RFC. These RFCs contain recommended procedure for Internet sites                                                                                                                         |
...
| Hostname                                 | A human-readable name assigned to a host on a network. An hostname is just a convenient shorthand for an IP address                                                                                |
...
| Internet Protocol (IP)                   | A network protocol that routes packets from one host to another. There are two version of IP, i.e., IPv4 and IPv6                                                                                  |
...
| Man-in-the-middle (MITM) attack          | An attack where the where the attacker secretly relays and possibly alters the communications between two parties who believe that they are directly communicating with each other                 |
| Maximum transmission unit (MTU)          | The largest size in bytes that can be transmitted in a single network later transaction                                                                                                            |
