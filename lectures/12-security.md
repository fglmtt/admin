...
As a system administrator you bear a heavy burden. You must push an agenda that secures your organization's systems and networks, ensures that they are vigilantly monitored, and properly educates your users and stuff, familiarize yourself with current security technology, and work with experts to identify and resolve vulnerabilities at your site
...
However, this is also a perfect example of the need to balance security against usability (see [[#1. Security facts|§1]]). If the system were unintentionally rebooted (e.g., after a power outage), an administrator would have to be physically there to get the computer running again
...
A backup is a copy of computer data taken and stored elsewhere so that it may be used to restore the original after a data loss event. Regular, tested system backups are an essential part of any site security plan. Backups fall into the "availability" bucket fo the CIA triad

Make sure that
- All filesystems are replicated
- Some backups are stored off-site

However, backups can also be a security hazard. Protect backups by limiting (and monitoring) access and by encrypting backups files
...
MFA systems are a good choice. These systems validate your identity both through something you know (a password or passphrase) and something you have (a physical device)
...
[Nmap](https://nmap.org/) is a network port scanner. Nmap also provides a publicly available host for safe scanning practice: [scanme.nmap.org](). Do not run Nmap on someone else's network without permission from one of that network's administrators
...
Cryptography applies mathematics to the problem of securing communications. A cryptographic algorithms, aka a cipher, is the set of mathematical steps taken to secure a message
...
The SSH system is a protocol for remote logins and for securing network services on an insecure networks. SSH capabilities range from remote command execution to port forwarding and file transfer. SSH is an indispensable tool for system administrators
...
| `POSTROUTING` | Outgoting      | After routing, just before packets leave the system     |
...
This command appends `(-A)` a rule to the `POSTROUTING` chain of the `nat` table (`-t`). This rule matches packets going out on the `eth1` interface (`-o`). The source IP address (`-j`) of the matching packets is changed to `128.138.101.4` (`--to`)
...
| Network port scanner                          | Software that that probes a target system to identify which network ports are open, closed, or filtered                                                                                                                                                                                                         |
